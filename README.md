...
- git branch -vv : 로컬 브랜치가 어디 원격브랜치에 연결되었는지 확인 , pull 할 경우 연결된 원격 브랜치로 pull한다. 
...
각자 기능 브랜치를 master브랜치로부터 만들고 그걸 merge request(pull request)하라고 하고 옵션은 mergerequest되면 그 브랜치는 삭제되도록 관리 이후 다시 master브랜치에서 새 기능 브랜치 따서 계속 작업하도록
...
1. 한 사람당 하나 main에서 본인 기능 브랜치 따서 가지고 있고
git branch <본인 기능 브랜치이름>

2. 본인 브랜치에서 작업하다가 주기적으로 본인 로컬 main브랜치는 원격 main브랜치로부터 pull받아주기
git checkout main 
git pull 

3. 자기 로컬 기능 브랜치에다 자기 로컬 main브랜치 merge해서 계속 작업
git checkout <본인 기능 브랜치이름>
git merge main

4. 충돌난거 있으면 수정하고 커밋, 이후 계속 본인 브랜치에서 작업

5. 본인브랜치에서 작업하다가 어느정도 완성이 되었으면 [본인 로컬 브랜치]  ==> [본인 원격 브랜치]에다 push하기
  그리고 [본인 원격 기능 브랜치]에서 [원격 main브랜치]에다 merge request날리기 
